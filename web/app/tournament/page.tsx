...
interface Matchup {
  ...
  position: number; // 0-6 (0=top, 3=bottom final)
}
      ...
      setTimeout(() => {
        setBracket(prev => {
          ...
          if (round < 3 && updated.winner) {
            const nextRoundId = round === 1
              ? (m.position < 2 ? 5 : 6)
              : 7;
            const slot = round === 1
              ? (m.position % 2 === 0 ? 'fighter1' : 'fighter2')
              : (m.position === 0 ? 'fighter1' : 'fighter2');
            return next.map(p => p.id === nextRoundId
              ? { ...p, [slot]: updated.winner }
              : p);
          }
          return next;
        });
    ...
    <div className="min-h-screen bg-dark-900">
      <div className="max-w-5xl mx-auto px-4 py-12">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="font-display text-3xl font-bold">🏆 Tournament</h1>
        </div>
              ...
              <div className="grid grid-cols-[1fr_1fr_auto_1fr] gap-4 items-center">
                ...
                <div className="space-y-4">
                  ...
                  {bracket.filter(m => m.round === 1).map(m => (
                    <BracketMatch key={m.id} match={m} isActive={animMatch === m.id} />
                  ))}
                </div>
                ...
                <div className="space-y-8">
                  ...
                  {bracket.filter(m => m.round === 3).map(m => (
                    <BracketMatch key={m.id} match={m} isActive={animMatch === m.id} isFinal />
                  ))}
                </div>
              </div>
  ...
  return (
    <motion.div
      ...
      <div className={`flex items-center gap-2 py-1 px-2 rounded ${winner?.id === f1?.id ? 'bg-green-500/10' : ''}`}>
        <span className="text-sm">{f1 ? { Delver: '⛏', Ironblood: '⚔', Forgeborn: '🔥', Shadowvein: '🗡', Stonewarden: '🛡', Veinbender: '🧪' }[f1.bloodline] : '❓'}</span>
        <span className={`text-xs truncate font-bold ${winner?.id === f1?.id ? 'text-green-400' : 'text-dark-300'}`}>
          {f1 ? f1.name : '???'}
        </span>
      ...
      <div className={`flex items-center gap-2 py-1 px-2 rounded ${winner?.id === f2?.id ? 'bg-green-500/10' : ''}`}>
        <span className="text-sm">{f2 ? { Delver: '⛏', Ironblood: '⚔', Forgeborn: '🔥', Shadowvein: '🗡', Stonewarden: '🛡', Veinbender: '🧪' }[f2.bloodline] : '❓'}</span>
        <span className={`text-xs truncate font-bold ${winner?.id === f2?.id ? 'text-green-400' : 'text-dark-300'}`}>
          {f2 ? f2.name : '???'}
        </span>
